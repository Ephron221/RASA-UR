    ...
    <div className="fixed bottom-10 right-10 z-[1000] flex flex-col gap-4 pointer-events-none">
      <AnimatePresence>
        {notifications.map((n) => (
          ...
          >
            {/* Background Glow */}
            <div className="absolute -inset-1 bg-gradient-to-r from-secondary/20 to-accent/20 rounded-[2rem] blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
            ...
            <div className="relative w-80 bg-white/90 backdrop-blur-2xl border border-white/50 shadow-[0_20px_50px_rgba(0,0,0,0.1)] rounded-[2rem] p-6 flex items-start gap-5 overflow-hidden">
               ...
               <div className={`shrink-0 w-12 h-12 rounded-2xl flex items-center justify-center shadow-lg ${
                 n.type === 'divine' ? 'bg-black text-secondary' : 'bg-secondary text-white'
               }`}>
                 {n.type === 'divine' ? <Sparkles size={24} /> : <CheckCircle2 size={24} />}
               </div>

               <div className="flex-grow space-y-1 pr-4">
                 <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-secondary">{n.title}</h4>
                 <p className="text-sm font-black text-black leading-tight">{n.message}</p>
                 <div className="flex items-center gap-2 pt-2">
                    <Heart size={10} fill="currentColor" className="text-red-400" />
                    <span className="text-[8px] font-black text-black/40 uppercase tracking-widest">Divine Handshake Confirmed</span>
                 </div>
               </div>

               <button 
                onClick={() => remove(n.id)}
                className="absolute top-4 right-4 p-1.5 hover:bg-gray-100 rounded-lg text-black/20 hover:text-red-500 transition-all"
               >
                 <X size={14}/>
               </button>
            </div>
