...
const DonationTab: React.FC<DonationTabProps> = ({ user }) => {
  ...
  const isExcom = user.role === 'executive';
  
  const canSeeProof = isAccountant || isIT || isExcom;
                    ...
                    {d.status === 'Pending' && (
                      canVerify ? (
                        <button 
                          ...
                          className="px-5 py-2.5 bg-emerald-500 text-white rounded-xl text-[9px] font-black uppercase shadow-lg shadow-emerald-100 hover:bg-emerald-600 transition-all active:scale-95 flex items-center gap-2"
                        >
