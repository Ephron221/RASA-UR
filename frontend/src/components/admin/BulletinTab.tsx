      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-6">
        <div className="space-y-1">
          <h3 className="text-3xl font-black font-serif italic text-black flex items-center gap-4">
            <Megaphone className="text-secondary" size={32} /> Bulletin Broadcast
          </h3>
          <p className="text-[10px] font-black uppercase tracking-widest text-black/40">Manage official campus mandates & info</p>
        </div>
      ...
      <div className="flex flex-col lg:flex-row gap-6 items-center justify-between bg-white p-5 rounded-[2.5rem] shadow-sm border border-gray-100">
        <div className="flex gap-2">
          {['All', 'Urgent', 'Notice', 'Info'].map(f => (
            <button key={f} onClick={() => setFilter(f as any)} className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${filter === f ? 'bg-black text-white shadow-lg' : 'text-black/40 hover:text-secondary hover:bg-gray-50'}`}>{f}</button>
          ))}
        </div>
        <div className="flex items-center gap-3 px-4 py-2 bg-secondary/5 rounded-full border border-secondary/10">
           <Zap size={14} className="text-secondary" />
           <span className="text-[9px] font-black text-secondary uppercase tracking-widest">{announcements.length} Total Sequences Stored</span>
        </div>
      </div>
          ...
          {filtered.map(a => {
            ...
            return (
              <motion.div 
                ...
                className={`group bg-white p-8 rounded-[3rem] border border-gray-100 shadow-sm hover:shadow-xl hover:shadow-secondary/5 transition-all flex flex-col md:flex-row items-center gap-8 ${!a.isActive ? 'opacity-60 grayscale-[0.5]' : ''}`}
              >
                ...
                <div className="flex-grow space-y-3 min-w-0">
                  <div className="flex items-center gap-4">
                    ...
                    <span className="text-[10px] font-black text-black/30 flex items-center gap-2"><Clock size={12}/> {a.date}</span>
                    <button 
                  ...
                  <h4 className="text-xl font-black text-black leading-tight truncate">{a.title}</h4>
                  <p className="text-sm text-black/40 font-medium line-clamp-1 leading-relaxed">{a.content}</p>
                </div>
                
                {canManage && (
                  <div className="flex gap-2 shrink-0 md:opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => onEdit(a)} className="p-4 bg-gray-50 text-black/30 rounded-2xl hover:bg-black hover:text-white transition-all shadow-sm active:scale-95" title="Modify"><Edit size={18}/></button>
